...
- `references/management-api.md` — programmatic control plane (models, deployments, envs, secrets). What `truss` CLI
  uses under the hood.
...
contradictions to the user rather than papering over them.
...
- Can't find something via docs MCP → `WebFetch` `baseten.co/llms.txt` or `docs.baseten.co/llms.txt` for an index, then
...
- **Read full pages via the `.md` URL, not the docs MCP.** Every docs page is also served as rendered markdown at
  `https://docs.baseten.co/<path>.md` (path = the page URL minus the domain, with `.md` appended; no auth). Faster,
  ~half the bytes, and bug-free. The MCP `cat`/`head` on `.mdx` pages **inflates content quadratically** on
  API-reference pages (each `<ParamField>` block is emitted with all prior siblings re-concatenated). Reserve the MCP
  filesystem tool for what it's actually good at: `rg`, `tree`, `find`, structural discovery — anything where you don't
  already know the page path.
- **`baseten_docs` MCP search is lexical.** "speech to text" can rank TTS above STT. When modality matters, query by
  path (`rg -l streaming-transcription /reference`) or use specific jargon.
