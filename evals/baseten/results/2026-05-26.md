# Baseten evaluation: May 26, 2026

Archived 320-execution Opus benchmark. All rows in [its statistics](stats.jsonl) carry timestamp `2026-05-26T18-26-52Z` and commit `ca33e74`. Earlier documentation incorrectly labeled this sweep April. The separate [April pilot](2026-04-17.md) used Sonnet and five prompts.

The findings and product guidance below describe the tested snapshot, including its now-superseded `model_cache` recommendation. They do not validate the September skill. Use the [evaluation guide](../README.md) for the latest results and run instructions. The original snapshot is identified by its recorded commit; the current harness and suite are not an exact reproduction of this experiment.
...
the suite recorded at commit `ca33e74` (the current suite has changed)):
...
tagged `core` or `nice`. The statistics retain both unweighted `pass_rate` and `weighted_pass_rate`; the tables below use `pass_rate`.
...
or cache. `cost_usd` is the cost reported by Claude Code, not a reconciliation against provider invoices.
