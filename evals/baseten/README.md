...
Source: [`harness/src/baseten_skills_evals/analyze.py`](harness/src/baseten_skills_evals/analyze.py).
...
```bash
# From evals/baseten/harness/
cd evals/baseten/harness
uv sync
...
`results/stats.jsonl` (one JSON row per cell) is committed to this repo as
the canonical source for the numbers above. Per-cell transcripts and grading
detail for the canonical sweep are kept outside the repo to stay small.
